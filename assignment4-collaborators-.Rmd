...
```{r lm-spending-visitor}
lm_spending_visitor <- lm(tour_exp_millionsusd ~ visitor_thousands + year, data = tour_crime)

glance(lm_spending_visitor)
```


```{r lm-spending-visitor, fig.cap = "Predicted value and Residual value (the model of visitor spending)}
augment(lm_spending_visitor, tour_crime) %>%  
  ggplot(aes(x = .fitted, 
             y = .resid)) +
  geom_point(alpha = 0.2) +
  xlab("Predicted value") +
