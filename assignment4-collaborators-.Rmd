...
![](/Users/ankushv/Desktop/assignment4-collaborators/Images/Map-of-Australia.jpg)
...
australia %>% 
  ...
  xlab("Year") + ylab("") +
  ggtitle("Tourism Expenditure vs Number of Visitors") +
  theme_test()
...
```{r}
tourist_arrivals <- australia %>% 
  dplyr::filter(Series == "Tourist/visitor arrivals (thousands)") %>% 
   dplyr::select(Country, Year, Value) %>%
   rename(Visitors = Value) 
```

```{r}
tourist_expenditure <- australia %>% 
  dplyr::filter(Series == "Tourism expenditure (millions of US dollars)") %>% 
  dplyr::select(Country, Year, Value) %>%
  rename(Expenditure = Value) 
```

```{r}
tourist_arr_exp <- full_join(tourist_arrivals,tourist_expenditure,
                             by = c("Country","Year")) 

tourist_arr_exp %>% 
  kableExtra::kable(booktabs = TRUE, caption = "Expenditure and Number of Visitors ('000) in Australia") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))

```

```{r}
tour <- tourist_arr_exp %>% 
  group_by(Year,Visitors, Expenditure) %>% 
  mutate(Visitors_Freq = cumsum(Visitors), Expenditure_Freq = cumsum(Expenditure)) %>% 
...
```{r}
Visit <- pie(tour$Visitors_per, labels = tour$Year,
             col = rainbow(length(tour$Year)),
             main = "Percentage of number of Visitors")
```

```{r}
Expen <- pie(tour$Expenditure_per, labels = tour$Year,
             col = topo.colors(length(tour$Year)),
             main = "Percentage of Expenditure in different Years")

```

```{r}
Growth_Visit <- tourist_arr_exp %>% 
  mutate(Visitors_Growth = c(0,48,5,43,7,5))

Growth_Expen <- tourist_arr_exp %>% 
  mutate(Expenditure_Growth = c(0,90,58,26,13,8))

```

```{r}
as.numeric(tourist_arr_exp$Visitors)
as.numeric(tourist_arr_exp$Expenditure)
