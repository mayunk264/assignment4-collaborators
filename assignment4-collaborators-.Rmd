...
However, when we as tourists think about visiting places one of the criteria, we look for is how safe is it to travel to that country/place? Visitors are most likely to visit safer places. In the context of international travel, tourists may defer from their visit, if the probability of victimization in that country is high. If this is one of the top checklists that visitors look out for, then the places with high crime rates will receive fewer visitors from abroad.referred: @2014crime
...
Although this data is form a well known data site, the actual crime rate within each country maybe faulty due to fear in constitutional laws within the country or corruption. The plot \@ref(fig:crimerates) depicts a column graph with year on the x-axis and counts on the y-axis, where the the years in the dataset vary from 2000 through to year 2016, looking at this plot it can be inferred that the rate of crimes in the year 2010 was the highest with a count of more than 750 mean cases throughout the world. Followed by approximate values in 2005, 2015 and 2016 with mean counts of 760, 600 and 500. It also illustrates that with the years the crime rates have been fluctuating rapidly but also with time the crime mean counts have decreased.
...
```{r worldmapdata, fig.height= 9 ,fig.pos="H", fig.width=9, fig.cap="World map depicting the crimes within each country"}
library(viridis)
...
In this section, we attempt to explore the crime results in each state and gender trend for crime offence in Australia. Regarding the Australia Bureau of Statistics (ABS) data sources, the following data sets are used to  analysis this section which is <https://www.abs.gov.au/AUSSTATS/abs@.nsf/DetailsPage/4519.02018-19?OpenDocument>,  <https://www.abs.gov.au/AUSSTATS/abs@.nsf/DetailsPage/4519.02016-17?OpenDocument>.
...
In this section we try to find the relationship between tourism and crime. For tourism, the number of visitor arrivals and visitor spending are two important indicators, therefore, we built two models for both of the indicators, respectively. Before creating models, we must remove the variables with too many missing values. Figure \@ref(fig:vismiss) specifies the missing values in crime data set. We only kept Intentional homicide rates per 100,000 and removed other variables. Table \@ref(tab:jointables) is table combined with crime and tourism. The missing values still existed, but they are few.
...
Next, we built three linear models for visitor spending. Three models all used country and year as explanatory variables. However, in Model 1, we used intentional homicide rates as additional explanatory variable; in Model 2, we used visitor arrivals as additional explanatory variable; in Model 3, we used both intentional homicide rates and visitor arrivals. Table \@ref(tab:lmspendingvisitor) shows Model 3 had the highest $R^2$ value. There were 96.54% of variations in explanatory variables could be explained by model 2. In sum, the number of visitor arrivals had bigger influence than crime rate on the fitness of the model. That is, holding crime rate and year constant, some countries were more attractive to tourists.
...
# Conclusion
(Varsha) In conclusion, the crime rates for every country is a problem, it decrease the boost in the economy and with the world map we could see that many countries have a high rate of crimes. In which the most commonly committed crime is Robbery at the national level,rate per 100,000 population and the country being Canada.

(Yawen) Overall, crime offence rates in Australia of each state are different from 2016 to 2019. The states with the highest crime rate are National Territories for drug offences, acts intended to cause injury, theft and public order offences. Then, crime rate of male is higher than female in each type of offence, but they have same trend in recent ten years. 

(Mayunk) Tourism remains an industry vital to the global economy, evidenced by the incredible growth in both number of tourists and the money they spend while on tour. France, Spain and the U.S. have been and still are the most popular tourist destinations. Tourists themselves spend the most money in Australia at over 5000 USD per person.

(Yuheng) The relationship between crime and tourism does exist. However, crime rate is not dominant explanatory variable in both linear models (models of visitor arrivals and visitor spending). Instead, country is the dominant variable in the model of visitor arrival while the visitor arrival is the dominant explanatory variable in the model of visitor spending. We may deduce that holding crime rate and year constant, some countries could attract more tourists than other countries. This is logical because some countries have more landmarks and viewpoints than other countries. In additional, due to the high $R^2$ values, two linear models may be used for predicting visitor arrival and visitor spending in the future. 



# Acknowledgments
The packages used to produce this report are: 
ggplot2 [@ggplot2], broom [@broom], naniar [@naniar], readxl[@readxl], ggthemes[@ggthemes]
