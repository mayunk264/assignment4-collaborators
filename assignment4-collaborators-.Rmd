...
crime_rate <- crime_world %>% filter(Year %in% c("2005","2010","2015","2016")) %>% group_by(Year,Crimes,region) %>% 
   count(Crimes)
ggplot(crime_rate,
       aes(x = Year,
           y = n)) + 
...
data <- crime_world %>%  
  select(Year,Crimes,Value,region) %>%  filter(Year %in% c("2005","2010","2015","2016")) %>%
  group_by(region) %>% pivot_wider(names_from = "Crimes", values_from = "Value") 
#ggplot(aes(x = region)) + geom_histogram(stat = "count") 
```


```{r}
pq <- data %>%  select(Year,`Intentional homicide rates per 100,000`,region) %>% filter(Year == "2010") %>% group_by(region,Year)  %>%  arrange(`Intentional homicide rates per 100,000`) %>%  top_n(7)
#ggplot(pq, aes(x = region))
```
...
p <- ggplot(crime_world_map, 
       aes(x = long, 
           y = lat, 
           group = group
           ),fill = Crimes) +
  geom_polygon(color = "grey90")+theme_classic()+
